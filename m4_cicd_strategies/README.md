...
passes its checks is automatically packaged and shipped — to a staging environment (Delivery) or
...
* **Pre-commit hooks** run on your machine, before a commit is even created — the cheapest place to
  ...
  files. In an emergency, `git commit --no-verify` skips the hooks — a habit worth using rarely and
...
* **CML** (§9) is the layer unique to ML: automated checks on the *training run itself* — did it
  converge, did a tracked metric actually improve, is this the right dataset version.
...
* `on` — the trigger: `push`, `pull_request`, `schedule`, `workflow_dispatch`, each scoped to branches
  or paths
...
different axis — it opens PRs automatically when a dependency (or an Action's own pinned version) falls
out of date:
